          ...
          <form action="http://mertest.chinapnr.com/cash/cashier/charge/desktop/routingPayChannel" method="post" name="chargeVoForm" id="chargeVoForm" class="errFocus">
            ...
            <div class="form form-border mb30">
              ...
              <div class="form-title form-title-big">
                <p class="info mb15"><span>平台名称: 此地有银</span>
                <span>公司名称: 此地有银运营(赣州)有限公司</span></p>
                <p class="info"><span> 银行账号：cdyy_12345678</span></p>
              </div>
              <!--end-->
                      ...
                      <p>个人网银：</p>
                        <div class="bank-list">
                            <select id="rbank" name="rbank">
                                <option value="0">${bank.get(0).getBcardnumber()}</option>
			                </select>
                        </div>
                      <p>&nbsp;</p>
                    </div>
...
<div class="secure-tips mb30">
  ...
  <script src="${pageContext.request.contextPath}/js/huifujs/plugins.min.js"></script>
  <script src="${pageContext.request.contextPath}/js/huifujs/main.js"></script>
 ...
 /* jQuery(document).ready(function() {
	...
	function article_edit(id){
	    //alert(id);
		var rbank=$("#rbank").val();
		$.post("<%=path%>/recharge/update?rid="+id+"&rbank=${bank.get(0).getBcardnumber()}"+"&rmoney=${money }",
			function(data){
				if(data.result=="fail"){
				}else if(data.result=="seccuss"){
					alert("充值成功");
					window.location.href="<%=path %>/left?uid=${customer.uid}";
				}
		})
	}
  </script>  
<script type="text/javascript" src="${pageContext.request.contextPath}/js/huifujs/useragents.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/huifujs/jQuery.md5.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/huifujs/postbe.js"></script>
