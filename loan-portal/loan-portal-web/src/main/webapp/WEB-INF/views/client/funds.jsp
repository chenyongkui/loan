...
<script type="text/javascript">
	function tixianq(){
		...
		var available = $("#cash-available").val();
...
<body bgcolor="#FFFFCC">
		...
		<div class="account-left">
			<div class="account-left-nav">
				...
				<ul class="sub-nav">
					...
					<li class="active"><a href="<%=path%>/funds">提现</a></li>
	...
	<div class="sub-a-nav">
		...
		<a href="<%=path %>/withdrawalsList?pageSize=5&page=1" class="active show-cash">提现记录</a>
	</div>
	<!-- <em class="em-line"></em> -->
</div>
<div class="account-content" style="display: block;">
	<!-- 提现 -->
	<form action="<%=path %>/withdrawals" method="post">
	<div class="cash-pay">
		<p class="tips-title">
		...
		<div class="pay-from">                                          
			...
			<div class="label cl" id="usableSum">
				...
				<label class="long">可提现金额： </label><input class="text" id="cash-available" value="0.0" readonly/>
				</c:if>
				<c:if test="${!bank.isEmpty()}">
				<label class="long">可提现金额： </label><input class="text" id="cash-available" value="${capital.available}" readonly/>
				</c:if>
			</div>
			
			<div class="label cl">
				<label class="long">提现至银行：</label>
				<%--<input value="${bank.get(0).baccount}" readonly/>--%>
				<c:if test="${!bank.isEmpty()}">
				<select>
					<option value="${bank.get(0).bid}">${bank.get(0).baccount}</option>
				</select>
				</c:if>
			</div>
