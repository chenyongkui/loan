...
<div class="zxcf_top_wper">
	<div class="zxcf_top px1000 clearfix">
		 ...
		 <div class="zxcf_top_r fr">
		 	...
		 	<c:if test="${customer.uid != null}">
			 	<span>欢迎</span>
			 	<a href="<%=path %>/left?uid=${customer.uid}">${customer.username}</a>
			 	<span>|</span>
		 	...
		 	<c:choose>
				<c:when test="${customer.uid == null}">
					<a href="<%=path %>/login">充值</a>
				</c:when>
				<c:when test="${customer.uid != null}">
					<a href="<%=path%>/recharge">充值</a>
				</c:when>
			</c:choose>
		 	<span>|</span>
...
<div class="zxcf_menu_wper" >
	<div class="zxcf_menu px1000" style="margin-left: 21.5%;">
		  <a href="<%=path %>/index1" class="zm_cura">首页</a>
		  <a href="<%=path %>/invest?page=1&rows=10">我要投资</a>
		  <a href="<%=path %>/borrow">我要借款</a>
		  <a href="<%=path %>/problem">新手指引</a>
		  <a href="<%=path %>/AboutUs" style="margin-right:0;">关于我们</a>

		<%--<c:if test="${customer.uid == null}">
					<li><a href="<%=path %>/login">我的账户</a></li>
				</c:if>
			<c:if test="${customer.uid != null}">
				 <a href="<%=path %>/left">我的账户</a>
			</c:if>--%>
...
<script>
	...
	function MyAccount(){
		...
		if(uid!=null){
			window.location.href='<%=path %>/left';
		}else{
