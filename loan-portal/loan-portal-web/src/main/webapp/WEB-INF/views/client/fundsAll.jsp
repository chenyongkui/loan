...
<link rel="stylesheet" href="<%=path%>/css/lanrenzhijia.css" media="all">
...
<body bgcolor="#FFFFCC">
	<jsp:include flush="true" page="top.jsp"></jsp:include>
		...
		<div class="account-left">
			<div class="account-left-nav">
				...
				<ul class="sub-nav">
					<li><a href="<%=path %>/left?uid=${customer.uid}">账户总览</a></li>
					<li><a href="<%=path%>/recharge">充值</a></li>
					<li class="active"><a href="<%=path%>/client/funds">提现</a></li>
					<li><a href="<%=path%>/MoneyRecord">资金记录</a></li>
				</ul>
	...
	<div class="sub-a-nav">
		<a href="<%=path%>/funds" class="active show-cash">提现</a>
		<a href="<%=path %>/withdrawalsList?pageSize=5&page=1">提现记录</a>
	</div>
	<!-- <em class="em-line"></em> -->
</div>
<div class="account-content" style="display: block;">
	...
	<div id="angodiv1">
		<table class="table" style="border: 2px solid; border-color: #DFF0D8;">
			...
			<tbody>
				<c:forEach var="list" items="${witList}">
					<tr class="success">
						<td>${customer.username}</td>
						<td>${bank.get(0).bcardnumber}</td>
						<td>${bank.get(0).baccount}</td>
						<td>${list.wmoney}</td>
	...
	<div style="margin-top: 5px; text-align: center;">
		<a href="<%=path %>/withdrawalsList?pageSize=5&page=1"  style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">首页</a>&nbsp;&nbsp;
		<c:if test="${(Page.page - 1)==0}">
			<a href="<%=path %>/withdrawalsList?pageSize=5&page=1"  style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">上一页</a>&nbsp;&nbsp;
		</c:if>
		<c:if test="${(Page.page - 1)!=0}">
		<a href="<%=path %>/withdrawalsList?pageSize=5&page=${Page.page - 1}"  style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">上一页</a>&nbsp;&nbsp;
		</c:if>

		<c:forEach begin="1" end="${(Page.count)}" varStatus="sta">

			<a href="<%=path %>/withdrawalsList?pageSize=5&page=${sta.count}"  style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">[${sta.count}]</a>&nbsp;&nbsp;
		</c:forEach>



		<c:if test="${(Page.page+1)>(Page.count)}">
			<a href="<%=path %>/withdrawalsList?pageSize=5&page=${Page.count}" onclick="mygoods(3)" style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">下一页</a>&nbsp;&nbsp;
		</c:if>
		<c:if test="${(Page.page +1)<=(Page.count)}">
		<a href="<%=path %>/withdrawalsList?pageSize=5&page=${Page.page + 1}" onclick="mygoods(3)" style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">下一页</a>&nbsp;&nbsp;
		</c:if>
		<a href="<%=path %>/withdrawalsList?pageSize=5&page=${Page.count}"  style="background:#224762;color:white;font-size:15px;border-radius:5px;text-decoration:none;">末页</a>&nbsp;&nbsp;
		当前页 第<span id="page">${Page.page}</span>页&nbsp;&nbsp;
		共<span id="count">${Page.count}</span>页&nbsp;&nbsp;
